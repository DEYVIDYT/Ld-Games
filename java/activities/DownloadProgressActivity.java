...
public class DownloadProgressActivity extends AppCompatActivity implements
        ...
        CompletedDownloadAdapter.OnDownloadActionListener { // Implementar o novo listener
    ...
    private CompletedDownloadAdapter completedAdapter; // Manter referência ao adapter
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        refreshDownloads(); // Carrega ativos e concluídos
    ...
    public void onDownloadUpdated(DownloadInfo downloadInfo) {
        runOnUiThread(() -> {
            ...
            for (int i = 0; i < downloads.size(); i++) {
                if (downloads.get(i).getUrl().equals(downloadInfo.getUrl())) {
                    index = i;
                    break;
                }
            }
            ...
            if (index != -1) {
                ...
                // Calcular velocidade e ETA usando os métodos implementados em DownloadInfo
                existingInfo.calculateSpeed();
                existingInfo.calculateEstimatedTimeRemaining();
    ...
    // Método auxiliar para atualizar a lista de concluídos
    private void refreshCompletedDownloads() {
        if (completedAdapter != null) { // Verificar se o adapter não é nulo
            List<DownloadInfo> completedList = downloadManager.getCompletedDownloads();
            completedAdapter.updateDownloads(completedList); // Chamar o método corrigido
            // A visibilidade é atualizada em refreshDownloads() ou updateTabVisibility()
        }
    }
    ...
    public void onCancel(DownloadInfo downloadInfo) {
        DownloadInfo currentInfo = downloadManager.getDownloadInfo(downloadInfo.getUrl());
    ...
    public void onDownloadDeleted(DownloadInfo downloadInfo) {
        // A exclusão já foi tratada no adapter, apenas atualizamos a visibilidade aqui
    ...
    private void showDownloadDetailsDialog(DownloadInfo downloadInfo) {
        ...
        if (startTime > 0) {
            ...
            tvDateAdded.setText(sdf.format(new Date(startTime)));
        } else {
            tvDateAdded.setText("Não disponível");
        }
    ...
    private long calculateAverageSpeed(DownloadInfo downloadInfo) {
        ...
        long startTime = downloadInfo.getStartTime();
        long endTime = downloadInfo.getEndTime();

        if (endTime == 0 && (downloadInfo.getStatus() == DownloadInfo.STATUS_RUNNING || downloadInfo.getStatus() == DownloadInfo.STATUS_RESUMING)) {
            endTime = System.currentTimeMillis();
        }

        if (downloadedSize <= 0 || startTime <= 0 || endTime <= startTime) {
            return 0;
        }
        ...
        return downloadedSize / totalTimeSeconds;
    }
