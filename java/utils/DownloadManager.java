...
public class DownloadManager {
    ...
    public void resumeDownload(DownloadInfo downloadInfo) {
        ...
        if (activeDownload != null && activeDownload.getStatus() == DownloadInfo.STATUS_PAUSED) {
            Log.d(TAG, "Retomando download (colocando na fila): " + activeDownload.getFileName());
    ...
    // Correção Erro 2: Tornar público
    public int getConcurrentDownloadLimit() {
        // Ler das preferências ou retornar padrão
        SharedPreferences prefs = context.getSharedPreferences("app_settings", Context.MODE_PRIVATE);
        return prefs.getInt("concurrent_downloads", DEFAULT_CONCURRENT_DOWNLOADS);
    }
